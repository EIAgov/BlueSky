...
def clean_storage_data(util):
    ...
    df = pd.read_csv(util.base_dir + util.inputKeys.loc['StorageINJCost', 'path'], index_col=0, header=0)
    ...
    df.rename(columns={'technology': 'storageTech', 'StorageINJCost':'base_StorageINJCost'}, inplace=True)
    util.write_aimms_output(df, ['base_StorageINJCost'], util.base_dir + util.outputKeys.loc['StorageINJCost', 'path'], note='Sourced from FECM', unit='millions of BasisYear_$/yr')
    
    #storage withdrawal costs
    
    df = pd.read_csv(util.base_dir + util.inputKeys.loc['StorageWTHCost', 'path'], index_col=0, header=0)
    ...
    df.rename(columns={'technology': 'storageTech', 'StorageWTHCost':'base_StorageWTHCost'}, inplace=True)
    util.write_aimms_output(df, ['base_StorageWTHCost'], util.base_dir + util.outputKeys.loc['StorageWTHCost', 'path'], note='Sourced from FECM', unit='BasisYear_$/kg mile')
    ...
    util.write_aimms_output(df, ['base_TransportationCost'], util.base_dir + util.outputKeys.loc['TransportationCost', 'path'], note='Sourced from FECM', unit='BasisYear_$/kg mile')
    ...
    util.write_aimms_output(df, ['base_SUCCost'], util.base_dir + util.outputKeys.loc['SUCCost', 'path'], note='Sourced from FECM', unit='BasisYear_$/kg mile')
    ...
    util.write_aimms_output(df, ['base_TUCCost'], util.base_dir + util.outputKeys.loc['TUCCost', 'path'], note='Sourced from FECM', unit='BasisYear_$/kg mile')
...
def clean_pipeline_data(util):
    ...
    df = pd.read_excel(util.base_dir + util.inputKeys.loc['pipe_cost', 'path'], index_col=None, header=0)
    costs = df.copy()[['r1', 'r2', 'Energy Intensity (kWh/kg h2)', 'Total Capital Cost']]
    costs.replace({'New England': 'NewEngland',
        ...
        'Mountain': 'Mountain',
        'Pacific': 'Pacific'}, inplace=True)
    costs.rename(columns={'r1': 'regCen', 'r2': 'regCen_j', 'Energy Intensity (kWh/kg h2)': 'pipe_energy_intensity', 'Total Capital Cost': 'pipe_tcc' }, inplace=True)

    util.write_aimms_output(costs, ['pipe_energy_intensity', 'pipe_tcc'], util.base_dir + util.outputKeys.loc['pipeline_costs', 'path'], note='Calculated as representative 18 inch pipelines', unit='TCC in 2023 USD and energy intensity in kWh/kg h2')
