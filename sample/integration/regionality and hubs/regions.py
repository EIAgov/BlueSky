...
class Functions:
    ...
    def get_elec_price(self,region,year):
...
class Model:
    ...
    def __init__(self,grid, mode, pricesblock):
    ...
    def generate_parameters(self,m):
        ...
        m.electricity_consumption_rate = Param(m.technology, initialize = 54.3)
        ...
        m.electricty_price = Param(m.regions,m.year, mutable = True, initialize = {(region,year): self.functions.get_elec_price(region,year) for region in m.regions for year in m.year})
    ...
    def solve(self, m):
        ...
        for tech in m.technology:
            self.production_stats['electricity_consumption_' + tech] = self.production['volume']*m.electricity_consumption_rate[tech]
