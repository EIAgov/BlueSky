...
class Functions:
    ...
    def get_production_cost(self,hub,tech,year):   
        ...
        if self.mode == 'standard':
            ...
            if tech == 'PEM': return self.registry.hubs[hub].region.data['electricity_cost']*45
            elif tech == 'SMR': return self.registry.hubs[hub].region.data['gas_cost']
            else: return 0
    ...
    def get_demand(self, region, time):
        ...
        if self.mode == 'standard':
            ...
            return self.registry.regions[region].get_data('demand')*1.10**(time - self.start_year)
...
class Model:
    ...
    def generate_parameters(self,m):
        ...
        m.demand = Param(m.regions,m.year, mutable=True, initialize = {(region,year): self.function.get_demand(region,year)})
        m.electricty_price = Param(m.hub,m.year, mutable = True, initialize = {(hub,year): self.function.get_elec_price(region,year)})
    ...
    def capacity_constraint(self,m,hub,tech,year):
        ...
        return m.h2_volume[hub,tech,year] <= self.functions.get_capacity(hub, tech) + sum(m.capacity_expansion[hub,tech,year] for year in range(max(m.year[1],year-20),year))
    ...
    def demand_constraint(self,m,region,year):
        ...
        if len(self.regions[region].hubs) == 0: 
            ...
            return sum(self.mass_balance(m,hub,year) for hub in self.regions[region].hubs.keys()) == self.functions.get_demand(region,year)
