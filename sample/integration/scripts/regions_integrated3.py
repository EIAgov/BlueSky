...
class Grid:
    ...
    def load_hubs(self):
        ...
        for index, row in self.data.hubs.iterrows():
            ...
            self.create_hub(row['hub'], self.registry.regions[row['region']], data = pd.DataFrame(row[2:]).transpose().reset_index())
...
class Data:
    ...
    def __init__(self):
        ...
        self.regions = pd.read_csv('C:/Users/MLJ/Downloads/H2/regionality and hubs/regions_single_test.csv', index_col = False)
        self.hubs = pd.read_csv('C:/Users/MLJ/Downloads/H2/regionality and hubs/hubs_single_region.csv', index_col = False)
        self.arcs = pd.read_csv('C:/Users/MLJ/Downloads/H2/regionality and hubs/transportation_arcs_single_region.csv', index_col = False)
        '''
        ...
        '''
        params = pd.ExcelFile('C:/Users/MLJ/Downloads/H2/regionality and hubs/parameter_list.xlsx')
...
class Functions:
    ...
    def get_electricty_consumption(self,m,region,year):
        ...
        return sum(m.electricity_consumption_rate[tech]*m.h2_volume[hub,tech,year] for tech in m.technology for hub in self.model.regions[region].hubs.keys())
    ...
    def get_production_cost(self,m, hub,tech, year):   
        ...
        if self.mode == 'standard':
            ...
            if tech == 'PEM': return m.electricity_price[self.model.hubs[hub].region.name,year]*m.electricity_consumption_rate[tech]
            elif tech == 'SMR': return m.gas_price[self.model.hubs[hub].region.name,year] + m.electricity_price[self.registry.hubs[hub].region.name,year]*m.electricity_consumption_rate[tech]
            else: return 0
            
        elif self.mode == 'integrated':
            print(self.mode)
            if tech == 'PEM': return self.model.electricity_price[self.model.hubs[hub].region.name,year]*m.electricity_consumption_rate[tech]
            elif tech == 'SMR': return m.gas_price[self.model.hubs[hub].region.name,year] + self.model.electricity_price[self.model.hubs[hub].region.name,year]*m.electricity_consumption_rate[tech]
            else: return 0
    ...
    def get_elec_price(self,m,region,year):
        ...
        if self.mode == 'standard': 
            if self.model.regions[region].data is None:
                return 0
            else: return self.model.regions[region].get_data('electricity_cost')
    ...
    def get_gas_price(self,m, region, year):
        ...
        else: return self.model.regions[region].get_data('gas_cost')
...
class Model:
    ...
    def start_build(self, block = False):
        ...
        if self.mode == 'integrated':
            self.m.production_cost.deactivate()
    ...
    def generate_parameters(self,m):
        ...
        m.electricity_consumption_rate = Param(m.technology, initialize = self.get_electricity_consumption_rate)
        ...
        if self.mode == 'standard':
            ...
            m.electricity_price = Param(m.regions,m.year, mutable = True, initialize = self.functions.get_elec_price)
            m.cost = Param(m.hubs,m.technology,m.year, mutable=True, initialize = self.functions.get_production_cost)
            ...
            '''
            #m.demand = self.demand
            #m.electricity_price = self.electricity_price
            #m.demand = Param(m.regions,m.year, mutable=True, initialize = self.demand)
            #m.electricity_price = Param(m.regions,m.year, mutable = True, initialize = self.electricity_price)
            m.cost = Param(m.hubs,m.technology,m.year, mutable=True, initialize = self.functions.get_production_cost)
    ...
    def capacity_constraint(self,m,hub,tech,year):
        ...
        return m.h2_volume[hub,tech,year] <= m.capacity[hub, tech] + sum(m.capacity_expansion[hub,tech,year] for year in range(max(m.year[1],year-30),year))
    ...
    def demand_constraint(self,m,region,year):
        ...
        if len(self.regions[region].hubs) == 0: 
            ...
            #return sum(self.mass_balance(m,hub,year) for hub in self.regions[region].hubs.keys()) - m.demand[region,year] == 0
            return sum(self.mass_balance(m,hub,year) for hub in self.regions[region].hubs.keys()) - self.demand[region,year] == 0
    ...
    def update_exchange_params(self, new_demand=None,electricity_price=None):
        ...
        if new_demand is not None: self.update_demand(new_demand)
        if new_electricity_price is not None: self.update_electricity_price(new_electricity_price)
        
        self.m.cost.store_values({(hub, tech, year): self.functions.get_production_cost(hub,tech,year,self.m, electricity_price) for hub in self.m.hubs for tech in self.m.technology for year in self.m.year})
    ...
    def resolve(self,new_demand=None,new_electricity_price=None, test = False):
        ...
        if test == True:
            ...
            self.update_exchange_params({(region,year): self.m.demand[region,year].value*1.1 for region in self.m.regions for year in self.m.year},{(region,year): self.m.electricity_price[region,year]*1.1 for region in self.m.regions for year in self.m.year})
            self.solve(self.m)
        else:
