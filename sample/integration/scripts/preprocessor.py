...
class Sets:
    ...
    def __init__(self,years,regions):
        ...
        r_file = pd.read_csv('../input/r_can_line.csv')
...
if __name__ == "__main__":
    ...
    all_frames = {}
    all_frames, setC = preprocessor(setA,'../input/temp/')
    for key in all_frames:
        all_frames[key].to_csv('../output/temp/'+key+'.csv')

    new = {}
    for filename in os.listdir('../output/temp/'):
        #print(filename[:-4])
        f = os.path.join('../output/temp/', filename)
        if os.path.isfile(f):
            new[filename[:-4]] = pd.read_csv(f)

    old = {}
    for filename in os.listdir('../output/inputs/'):
        #print(filename[:-4])
        f = os.path.join('../output/inputs/', filename)
        if os.path.isfile(f):
