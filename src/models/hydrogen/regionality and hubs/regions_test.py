...
class Functions:
    def __init__(self, grid, mode='standard', start_year=2025, end_year=2050):
        self.grid = grid
    ...
    def get_electricty_consumption(self, m, region, year):
        return sum(
            m.electricity_consumption_rate[tech] * value(m.h2_volume[hub, tech, year])
            for tech in m.technology_s
            for hub in m.regions[region].hubs.keys()
        )

    def get_capacity(self, m, hub, tech):
        if self.mode == 'standard':
            return m.hubs[hub].get_data('production_capacity_' + tech)
    ...
    def get_production_cost(self, m, hub, tech, year, update=False):
        if self.mode == 'standard':
            if tech == 'PEM':
                return (
                    m.electricity_price[m.hubs[hub].region.name, year]
                    * m.electricity_consumption_rate[tech]
                )
            elif tech == 'SMR':
                return (
                    m.gas_price[m.hubs[hub].region.name, year]
                    + m.electricity_price[m.hubs[hub].region.name, year]
                    * m.electricity_consumption_rate[tech]
                )
            else:
    ...
    def get_elec_price(self, m, region, year):
        if self.mode == 'standard':
            if m.regions[region].data is None:
                return 0
            else:
                return m.regions[region].get_data('electricity_cost')
    ...
    def get_gas_price(self, m, region, year):
        if m.regions[region].data is None:
            ...
            return self.registry.regions[region].get_data('gas_cost')
    ...
    def get_demand(self, m, region, time):
        if self.mode == 'standard':
            if m.regions[region].data is None:
                ...
                return m.regions[region].get_data('demand') * 1.05 ** (time - self.start_year)
...
class Integrated_Model(Block):
    def __init__(self, grid, functions, mode='block'):
        self.grid = grid
        self.registry = grid.registry
    ...
    def start_build(self):
        ...
        # self.production_cost = Objective(rule = self.total_cost())

    def generate_sets(self):
        # self.hubs_s = Set(initialize = self.hubs.keys())
        # self.arcs_s = Set(initialize = self.arcs.keys())
        # self.regions_s = Set(initialize = self.regions.keys())
        # self.technology_s = Set(initialize = self.data.technologies)
        # self.year_s = RangeSet(self.functions.start_year, self.functions.end_year)
        pass
    ...
    def generate_parameters(self):
        ...
        self.capacity = Param(
            ...
            self.technology_s,
            initialize={
                (hub, tech): self.functions.get_capacity(self, hub, tech)
                for hub in self.hubs_s
                for tech in self.technology_s
            },
        )
        self.electricity_consumption_rate = Param(
            self.technology_s,
            initialize={
                tech: self.functions.get_electricity_consumption_rate(tech)
                for tech in self.technology_s
            },
        )
        self.gas_price = Param(
            self.regions,
            self.year,
            initialize={
                (region, year): self.regions[region].get_data('gas_cost')
                for region in self.regions_s
        ...
        self.demand = Param(
            ...
            mutable=True,
            initialize={
                (region, year): self.functions.get_demand(self, region, year)
                for region in self.regions_s
                for year in self.year_s
            },
        )
        self.electricity_price = Param(
            self.regions_s,
            self.year_s,
            mutable=True,
            initialize={
                (region, year): self.functions.get_elec_price(self, region, year)
                for region in self.regions_s
                for year in self.year_s
            },
        )
        self.cost = Param(
            ...
            mutable=True,
            initialize={
                (hub, tech, year): self.functions.get_production_cost(self, hub, tech, year)
                for hub in self.hubs_s
                for tech in self.technology_s
                for year in self.year_s
            },
    ...
    def generate_variables(self):
        self.h2_volume = Var(self.hubs_s, self.technology_s, self.year_s, within=NonNegativeReals)
        self.transportation_volume = Var(self.arcs_s, self.year_s, within=NonNegativeReals)
        self.capacity_expansion = Var(
            self.hubs_s, self.technology_s, self.year_s, within=NonNegativeReals, initialize=0.0
        )
        self.trans_capacity_expansion = Var(
            self.arcs_s, self.year_s, within=NonNegativeReals, initialize=0.0
        )

    def generate_constraints(self):
        self.capacity_constraint = Constraint(
            self.hubs_s, self.technology_s, self.year_s, rule=self.capacity_constraint
        )
        self.transportation_constraint = Constraint(
            self.arcs_s, self.year_s, rule=self.transportation_capacity_constraint
        )
        self.demand_constraint = Constraint(self.regions, self.year, rule=self.demand_constraint)

    def mass_balance(self, hub, year):
        return (
            sum(self.h2_volume[hub, tech, year] for tech in self.technology_s)
            + sum(
                self.transportation_volume[arc.name, year]
                for arc in self.hubs[hub].inbound.values()
            )
            - sum(
                self.transportation_volume[arc.name, year]
                for arc in self.hubs[hub].outbound.values()
            )
        )

    def transportation_capacity_constraint(self, origin, destination, year):
        return self.transportation_volume[(origin, destination), year] <= self.arcs[
            (origin, destination)
        ].capacity + sum(
            self.trans_capacity_expansion[(origin, destination), year]
            for year in range(self.year_s[1], year)
        )
        # return m.transportation_volume[(origin,destination),year] <= self.arcs[(origin,destination)].capacity

    def capacity_constraint(self, hub, tech, year):
        # return m.h2_volume[hub,tech,year] <= self.hubs[hub].data.iloc[0]['H2Capacity_' + tech] + m.capacity_expansion[hub,tech, year]
        return self.h2_volume[hub, tech, year] <= self.functions.get_capacity(hub, tech) + sum(
            self.capacity_expansion[hub, tech, year]
            for year in range(max(self.year_s[1], year - 30), year)
        )

    def demand_constraint(self, region, year):
        if len(self.regions[region].hubs) == 0:
            return Constraint.Feasible
        else:
            return (
                sum(self.mass_balance(hub, year) for hub in self.regions[region].hubs.keys())
                - self.demand[region, year]
                == 0
            )

    def production_cost(self):
        return sum(
            self.h2_volume[hub, tech, year] * self.cost[hub, tech, year]
            for hub in self.hubs_s
            for tech in self.technology_s
            for year in self.year_s
        )
    ...
    def transportation_cost(self):
        return sum(
            self.transportation_volume[arc, year] * 0.12
            for arc in self.arcs_s
            for year in self.year_s
        )

    def prod_capacity_expansion_cost(self):
        return sum(
            self.capacity_expansion[hub, tech, year] * 10
            for hub in self.hubs_s
            for tech in self.technology_s
            for year in self.year_s
        )

    def trans_capacity_expansion_cost(self):
        return sum(
            self.trans_capacity_expansion[arc, year] * 3
            for arc in self.arcs_s
            for year in self.year_s
        )

    def capacity_expansion_cost(self):
        return self.prod_capacity_expansion_cost() + self.trans_capacity_expansion_cost()

    def solve(self):
        solver = SolverFactory('appsi_highs')
        solver.solve(self, tee=True).write()
        ...
        self.production_stats = pd.DataFrame(
            {
                ...
                'volume': [
                    sum(
                        value(self.h2_volume[hub, tech, year])
        ...
        for tech in self.technology_s:
            self.production_stats['electricity_consumption_' + tech] = (
                self.production_stats['volume'] * self.electricity_consumption_rate[tech]
            )
        ...
        self.h2_price = pd.DataFrame(
            {
                'region': [
                    self.registry.hubs[hub].region.name
                    for hub in self.hubs_s
                    for year in self.year_s
                ],
                'year': [year for hub in self.hubs_s for year in self.year_s],
                'price': [
                    self.dual[self.demand_constraint[self.registry.hubs[hub].region.name, year]]
                    for hub in self.hub_s
                    for year in self.year_s
                ],
            }
        )
    ...
    def total_cost(self):
        return self.production_cost() + self.transportation_cost() + self.capacity_expansion_cost()
    ...
    def update_demand(self, new_demand):
        ...
        self.demand.store_values(new_demand)
    ...
    def update_electricity_price(self, new_electricity_price):
        ...
        self.electricity_price.store_values(new_electricity_price)
    ...
    def update_exchange_params(self, new_demand=None, new_electricity_price=None):
        ...
        self.cost.store_values(
            {
                (hub, tech, year): self.functions.get_production_cost(hub, tech, year, self)
                for hub in self.hubs_s
                for tech in self.technology_s
                for year in self.year_s
            }
    ...
    def resolve(self, new_demand=None, new_electricity_price=None, test=False):
        ...
        if test == True:
            self.update_exchange_params(
                {
                    (region, year): self.demand[region, year].value * 1.1
...
model = ConcreteModel()
model.block = Integrated_Model(grid, Functions(grid), 'standard')
