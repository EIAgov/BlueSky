...
HMM.TaxCreditV = Param(
    HMM.hour,
    HMM.Season,
    HMM.EMMRegion,
    HMM.PlanningPeriod,
    HMM.CapacityVintageYear,
    initialize=data.TaxCreditV_f,
)
HMM.CO2CaptureRate = Param(HMM.ProdTechTypeGas, initialize=data.CO2CaptureRate_f)
HMM.CO2Value = Param(
    HMM.CensusRegion, HMM.PlanningPeriod, HMM.CapacityVintageYear, initialize=data.CO2Value_f
)
HMM.ElectrolyzerFuelConsumption = Param(
    HMM.FuelType, HMM.ProdTechTypeElec, initialize=data.ElectrolyzerFuelConsumption_f
)
HMM.ExistingCapacityElectrolyzer = Param(HMM.EMMRegion, HMM.PlanningPeriod, HMM.CapacityVintageYear)
HMM.ExistingCapacityNonElectrolyzer = Param(
    HMM.CensusRegion, HMM.PlanningPeriod, HMM.CapacityVintageYear
)
...
HMM.FeedstockConsumption = Param(
    HMM.ProdTechTypeGas, HMM.FuelType, initialize=data.FeedstockConsumption_f
)
HMM.FuelCost = Param(
    HMM.Season, HMM.CensusRegion, HMM.FuelType, HMM.PlanningPeriod, initialize=data.FuelCost_f
)
...
HMM.HourlyFuelCost = Param(
    HMM.hour,
    HMM.Season,
    HMM.EMMRegion,
    HMM.FuelType,
    HMM.PlanningPeriod,
    initialize=data.HourlyFuelCost_f,
)
...
HMM.OMCostElectrolyzer = Param(
    HMM.hour,
    HMM.Season,
    HMM.EMMRegion,
    HMM.ProdTechTypeElec,
    HMM.PlanningPeriod,
    initialize=data.OMCostElectrolyzer_f,
)
HMM.OMCostNonElectrolyzer = Param(
    HMM.Season,
    HMM.CensusRegion,
    HMM.ProdTechTypeGas,
    HMM.PlanningPeriod,
    initialize=data.OMCostNonElectrolyzer_f,
)
HMM.ProductionStepCostMultiplier = Param(HMM.ProductionStep)
HMM.PUCCostElectrolyzer = Param(
    HMM.EMMRegion, HMM.ProdTechTypeElec, HMM.PlanningPeriod, initialize=data.PUCCostElectrolyzer_f
)
HMM.PUCCostNonElectrolyzer = Param(
    HMM.CensusRegion, HMM.ProdTechTypeGas, HMM.PlanningPeriod, initialize=data.PUC_nonElectrolyzer_f
)
HMM.SeasonalFraction = Param(HMM.Season, initialize=data.SeasonalFraction_f)
HMM.SUCCost = Param(
    HMM.CensusRegion, HMM.StorageTechnologyType, HMM.PlanningPeriod, initialize=data.SUCCost_f
)
HMM.StorageINJCost = Param(
    HMM.CensusRegion, HMM.StorageTechnologyType, initialize=data.StorageINJCost_f
)
HMM.StorageStepCostMultiplier = Param(HMM.StorageStep)
HMM.StorageWTHCost = Param(
    HMM.CensusRegion, HMM.StorageTechnologyType, initialize=data.StorageWTHCost_f
)
HMM.TotalElectricityGen = Param(HMM.hour, HMM.Season, HMM.EMMRegion, HMM.PlanningPeriod)
HMM.TransportationCost = Param(
    HMM.TransportationArcs, HMM.PlanningPeriod, initialize=data.TransportationCost_f
)
...
HMM.FlowHubToDemand = Var(
    HMM.Season, HMM.Hub, HMM.DemandNode, HMM.PlanningPeriod, within=NonNegativeReals
)
HMM.FlowSupplyToHub = Var(
    HMM.Season,
    HMM.SupplyNode,
    HMM.Hub,
    HMM.ProductionTechnologyType,
    HMM.PlanningPeriod,
    within=NonNegativeReals,
)


HMM.NetStorageToHub = Var(
    HMM.Season,
    HMM.CensusRegion,
    HMM.StorageTechnologyType,
    HMM.PlanningPeriod,
    within=NonNegativeReals,
)
HMM.PUCByStepElectrolyzer = Var(
    HMM.ProductionStep,
    HMM.EMMRegion,
    HMM.ProductionTechnologyType,
    HMM.PlanningPeriod,
...
HMM.PUCByStepNonElectrolyzer = Var(
    HMM.ProductionStep,
    HMM.CensusRegion,
    HMM.ProductionTechnologyType,
    HMM.PlanningPeriod,
...
HMM.TUCByStep = Var(
    HMM.TransportationStep, HMM.TransportationArcs, HMM.PlanningPeriod, domain=NonNegativeReals
)
...
def electrolyzer_cost(model):
    return sum(
        ...
        for seas in model.Season
        for regNERC in model.EMMRegion
        for prodTech in model.ProdTechTypeElec
        for pp in model.PlanningPeriod
        for capyear in model.CapacityVintageYear
    )
...
def nonelectrolyzer_cost(model):
    return sum(
        (
            sum(
                model.FuelCost[seas, regCEN, fuel, pp]
                * (model.FeedstockConsumption[prodTech, fuel] + model.HPConsumption[fuel, prodTech])
                for fuel in model.FuelType
            )
        ...
        * model.H2ProdNonElectrolyzer[seas, regCEN, prodTech, pp, capYear]
        for seas in model.Season
        for regCEN in model.CensusRegion
        for prodTech in model.ProdTechTypeGas
        for pp in model.PlanningPeriod
        for capYear in model.CapacityVintageYear
    )
...
def demand_balance_constraint(model, Season, DemandNode, PlanningPeriod):
    return (
        sum(
            model.FlowHubToDemand[Season, Hub, DemandNode, PlanningPeriod]
            for Hub in data.HubsToDemandNode[DemandNode]
        )
        == model.H2Demand[Season, DemandNode, PlanningPeriod]
    )
...
def supply_mass_balance_constraint(model, Season, Hub, PlanningPeriod):
    return sum(
        sum(
            ...
            for hr in model.hour
            for seas in model.Season
            for regNERC in data.CensusToNERC[CensusRegion]
            for prodTech in model.ProdTechTypeElec
            for capYear in model.CapacityVintageYear
            for pp in model.PlanningPeriod
        )
        for CensusRegion in data.SupplyNodesToHub[Hub]
    ) + sum(
        model.H2ProdNonElectrolyzer[seas, regCEN, prodTech, pp, capYear]
        for seas in model.Season
        for regCEN in model.CensusRegion
        ...
        model.FlowSupplyToHub[seas, Hub, regCEN, prodtech, pp]
        for seas in model.Season
        for regCEN in model.CensusRegion
        for prodtech in model.ProductionTechnologyType
        for pp in model.PlanningPeriod
    )
...
HMM.DemandBalanceConstraint = Constraint(
    HMM.Season, HMM.DemandNode, HMM.PlanningPeriod, rule=demand_balance_constraint
)
HMM.SupplyMassBalanceConstraint = Constraint(
    HMM.Season, HMM.Hub, HMM.PlanningPeriod, rule=supply_mass_balance_constraint
)
