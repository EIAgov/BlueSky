...
def clean_storage_cap(util):
    ...
    df = pd.read_csv(
        util.base_dir + util.inputKeys.loc['saltCavernCapHist', 'path'], index_col=0, header=0
    )
    ...
    for i in df['Start']:
        ...
        temp['hist_storage_cap'] = temp['hist_storage_cap'] + int(
            df.loc[df['Start'] == i]['Volume_cubic_meters']
        )
        final.update(temp)

    # convert from cubic meters to kg of hydrogen
    final['hist_storage_cap'] = final['hist_storage_cap'] * 0.08
    ...
    util.write_aimms_output(
        final,
        ['hist_storage_cap'],
        util.base_dir + util.outputKeys.loc['hist_storage_cap', 'path'],
        note='multiple sources, see data readme',
        unit='kg of usable h2',
    )


def clean_smr_prod_cap_hist(util):
    # SMR with CCS cap history
    df = pd.read_excel(util.base_dir + util.inputKeys.loc['smr_ccs_cap', 'path'])[
        ['skip', 'Start_year', 'Project status', 'State', 'CO2 capacity (MMt/yr)']
    ].dropna()
    ...
    full_ccs = (
        full_ccs.merge(ccs[['histyear', 'r_cen', 'cap']], how='left', on=['histyear', 'r_cen'])
        .fillna(0)
        .drop('hist_cap', axis=1)
    )
    ...
    df = pd.read_excel(
        util.base_dir + util.inputKeys.loc['refinery_smr_hist', 'path'],
        sheet_name='Data 1',
        skiprows=1,
    )
    ...
    ref_state = (
        ref_state[['histyear', 'r_cen', 'cap']]
        .groupby(by=['histyear', 'r_cen'], group_keys=True, as_index=False)
        .sum()
    )
    # convert from million cubic feet per day to kg per year
    ref_state['cap'] = ref_state['cap'] * 1000000 * 365 * 0.00236
    ...
    tmp_ccs = full_ccs.loc[
        (full_ccs['histyear'] >= cap['histyear'].min()) & (full_ccs['histyear'] <= util.last_hYear)
    ]
    ...
    util.write_aimms_output(
        cap,
        ['histyear', 'hist_prod_cap'],
        util.base_dir + util.outputKeys.loc['hist_prod_cap', 'path'],
        note='EIA estimates',
        unit='annual kg of H2',
    )
    ...
    future_ccs = (
        future_ccs[['histyear', 'r_cen', 'prodTech', 'cap']]
        .groupby(['histyear', 'prodTech', 'r_cen'])
        .sum()
        .reset_index()
    )
    future_ccs.rename(
        columns={'histyear': 'i_calYear', 'r_cen': 'regCEN', 'cap': 'planned_cap'}, inplace=True
    )
    future_ccs = future_ccs[['i_calYear', 'regCEN', 'prodTech', 'planned_cap']]
    util.write_aimms_output(
        future_ccs,
        ['i_calYear', 'planned_cap'],
        util.base_dir + util.outputKeys.loc['planned_cap', 'path'],
...
def clean_electrolyzer_prod_cap_hist(util):
    ...
    elect_hist = elec_hist.merge(
        util.state_census_map, how='inner', left_on='State', right_on='reg'
    )
    ...
    elec_planned = elec_planned.merge(
        util.state_census_map, how='inner', left_on='State', right_on='reg'
    )
    ...
    util.write_single_value_param(
        cap[0],
        'electrolyzer_learning_cap',
        util.base_dir + util.outputKeys.loc['electrolyzer_learning_cap', 'path'],
        note='Global PEM With Estimated start construction <= 2023',
        unit='Annual kg of hydrogen. Max theoretical assuming 8760 hours of production a year',
    )
