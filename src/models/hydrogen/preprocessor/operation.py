...
def clean_census_indexed_op(util):
    ...
    df = pd.read_csv(
        util.base_dir + util.inputKeys.loc['OM_nonElectrolyzer', 'path'], index_col=0, header=0
    )
    ...
    df.rename(
        columns={'OM_cost': 'base_FixedOM_NonElectrolyzer', 'technology': 'prodTech'}, inplace=True
    )
    util.write_aimms_output(
        df,
        ['base_FixedOM_NonElectrolyzer'],
        util.base_dir + util.outputKeys.loc['FixedOM_NonElectrolyzer', 'path'],
    ...
    df = pd.read_csv(
        util.base_dir + util.inputKeys.loc['nonElectrolyzerVarOM', 'path'], index_col=0, header=0
    )
    df = index_df.merge(df.reset_index(), how='cross')
    df = df[['seas', 'regCen', 'prodTech', 'value']]
    df.rename(
        columns={'value': 'base_VarOM_NonElectrolyzer', 'technology': 'prodTech'}, inplace=True
    )

    util.write_aimms_output(
        df,
        ['base_VarOM_NonElectrolyzer'],
        util.base_dir + util.outputKeys.loc['VarOM_NonElectrolyzer', 'path'],
        note='Sourced from H2A. Variable OM less energy',
        unit='$/kg of H2 produced',
    )

    # 2 capacity expansion costs
    df = pd.read_csv(
        util.base_dir + util.inputKeys.loc['PUC_nonElectrolyzer', 'path'], index_col=0, header=0
    )
    ...
    df.rename(
        columns={'cap_cost': 'base_PUCCostNonElectrolyzer', 'technology': 'prodTech'}, inplace=True
    )

    util.write_aimms_output(
        df,
        ['base_PUCCostNonElectrolyzer'],
        util.base_dir + util.outputKeys.loc['PUC_nonElectrolyzer', 'path'],
        note='Sourced from H2A. discounted over lifetime with financials factored in',
        unit='$/kg H2',
    )
    ...
    df = pd.read_csv(
        util.base_dir + util.inputKeys.loc['OMCostElectrolyzer', 'path'], index_col=0, header=0
    )
    ...
    df.rename(
        columns={'OMCostElectrolyzer': 'base_FixedOM_Electrolyzer', 'technology': 'prodTech'},
        inplace=True,
    )

    util.write_aimms_output(
        df,
        ['base_FixedOM_Electrolyzer'],
    ...
    df = pd.read_csv(
        util.base_dir + util.inputKeys.loc['ElectrolyzerVarOM', 'path'], index_col=0, header=0
    )
    df = index_df.merge(df.reset_index(), how='cross')
    ...
    util.write_aimms_output(
        df,
        ['base_VarOM_Electrolyzer'],
        util.base_dir + util.outputKeys.loc['VarOM_Electrolyzer', 'path'],
        note='Sourced from H2A. Variable OM less energy',
        unit='$/kg of H2 produced',
    )
    ...
    df = pd.read_csv(
        util.base_dir + util.inputKeys.loc['PUCCostElectrolyzer', 'path'], index_col=0, header=0
    )
    ...
    df.rename(
        columns={'PUCCostElectrolyzer': 'base_PUCCostElectrolyzer', 'technology': 'prodTech'},
        inplace=True,
    )

    util.write_aimms_output(
        df,
        ['base_PUCCostElectrolyzer'],
    ...
    util.write_aimms_output(
        df,
        ['CO2CaptureRate'],
        util.base_dir + util.outputKeys.loc['CO2CaptureRate', 'path'],
        note='Sourced from H2A',
        unit='kg CO2/kg H2',
    )
