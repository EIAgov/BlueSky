...
def load_data_to_db_tablewise(session, table_name, table_class, dataframe):
    """Function that checks whether to delete and recreate each table in the cem db file based on a series of checks
    ...
    At each step, if check fails, delete the data and upload new dataframe. If table scheme (e.g. names) don't match, the function deletes and recreates the table schema as well as the data
    ...
    if check_names:
        ...
        print(f"Names for columns for {table_name} are the same; checking whether data shape is the same")
        if (table.shape == dataframe.shape):
            ...
            check_data = ~(dataframe.reset_index(drop=True) == table.reset_index(drop=True)).any().any()
            if check_data:
