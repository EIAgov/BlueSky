...
def preprocessor(setin,dir):
    ...
    def avg_by_group(set_name,map_df):
        #used to qa
        ...
        pos = all_frames[key].columns.get_loc(set_name)
        map_name = 'Map_'+set_name
        groupby_cols = list(all_frames[key].columns[:-1])+[map_name]
        groupby_cols.remove(set_name)

        #group df by year map data and update y col
        all_frames[key] = pd.merge(all_frames[key],map_df,how='left',on=[set_name])
        all_frames[key] = all_frames[key].groupby(by=groupby_cols,as_index=False).mean()
        all_frames[key][set_name] = all_frames[key][map_name]
        all_frames[key] = all_frames[key].drop(columns=[map_name]).reset_index(drop=True)

        #move back to original position
        y_col = all_frames[key].pop(set_name)
        all_frames[key].insert(pos, set_name, y_col) 
    ...
    for key in all_frames.keys():
        ...
        if 'y' in all_frames[key].columns:
            avg_by_group('y',y_map)
            #print(all_frames[key].tail())
        if 'hr' in all_frames[key].columns:
            avg_by_group('hr',df[['hr','Map_hr']])
            #print(all_frames[key].tail())
    ...
    #Creating temporal mapping parameters
    all_frames['Map_day_s'] = df[['Map_day','Map_s']].rename(columns={'Map_day':'day','Map_s':'s'}).drop_duplicates()
    all_frames['Map_hr_d'] = df[['Map_hr','Map_day']].rename(columns={'Map_day':'day','Map_hr':'hr'}).drop_duplicates()
    all_frames['Map_hr_s'] = df[['Map_hr','Map_s']].rename(columns={'Map_hr':'hr','Map_s':'s'}).drop_duplicates()
    all_frames['Hr_weights'] = df[['Map_hr','Hr_weights']].copy().rename(columns={'Map_hr':'hr'}).drop_duplicates()
    all_frames['Dayweights'] = df[['Map_hr','Dayweights']].copy().rename(columns={'Map_hr':'hr'}).drop_duplicates()
    all_frames['Idaytq'] = df[['Map_day','Dayweights']].copy().rename(columns={'Map_day':'day','Dayweights':'Idaytq'}).drop_duplicates()
...
if __name__ == "__main__":
    ...
    all_frames, setB = preprocessor(setA,'../input/cem_inputs/')
    for key in all_frames:
